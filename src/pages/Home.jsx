...
const Interactive3DCard = ({ title, description, modelSrc, to }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-black/30 transition-opacity duration-300 ${isHovered ? "opacity-0" : "opacity-100"}`}
      />
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-4">
        ...
        {description && (
          <p className="text-[10px] sm:text-xs text-slate-300 mt-0.5 line-clamp-2 m-0">
            {description}
          </p>
        )}
...
const SectionHeader = ({
  ...
  <div className="flex items-center justify-between gap-3">
    ...
    {linkTo && (
      <Link
        to={linkTo}
        className="text-sm font-medium text-indigo-600 hover:text-indigo-800 flex items-center gap-1 no-underline shrink-0"
      >
...
const Home = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const tags = (Array.isArray(tagRes?.data) ? tagRes.data : [])
          ...
          .sort((a, b) => {
            if (a.isMainTag !== b.isMainTag)
              return (b.isMainTag ? 1 : 0) - (a.isMainTag ? 1 : 0);
  ...
  const heroModelSrc =
    suggestedProducts[0]?.modelSrc ||
    categoryCards[0]?.modelSrc ||
    FALLBACK_GLBS[0];
    ...
    <div className="space-y-10">
      ...
      {allVariants.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div
              key={stat.label}
              className="rounded-lg bg-white border border-slate-200 px-4 py-3 text-center shadow-sm"
            >
              <p className="text-lg sm:text-xl font-bold text-indigo-600 m-0">
                {stat.value}
              </p>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[240px_minmax(0,1.7fr)_minmax(0,0.9fr)] gap-5">
        <aside className="hidden lg:block rounded-xl bg-white shadow-sm border border-slate-200 overflow-hidden">
          ...
          <ul className="divide-y divide-slate-100 text-sm max-h-[420px] overflow-y-auto">
            {conceptTags.length === 0 ? (
              <li className="px-4 py-3 text-slate-400 text-xs">
                Chưa có danh mục
              </li>
            ) : (
              conceptTags.map((tag) => (
          ...
          <div className="relative px-8 py-8 sm:px-10 sm:py-10 flex flex-col gap-6 sm:flex-row sm:items-center">
            <div className="flex-1 space-y-3 sm:space-y-4">
              ...
              <p className="text-sm sm:text-base text-indigo-100 max-w-xl m-0">
                Từ mô hình sưu tầm đến linh kiện kỹ thuật, chọn vật liệu PLA /
                PETG / Resin và đặt in theo yêu cầu.
              </p>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl bg-slate-900 text-slate-50 p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-emerald-300 m-0">
              Hỗ trợ nhanh
            </p>
            <p className="mt-1 text-sm font-semibold m-0">
              Tư vấn file & vật liệu miễn phí
            </p>
            <p className="mt-2 text-xs text-slate-300 m-0">
              Gửi file STL/OBJ/GLB hoặc ảnh tham khảo — kỹ thuật viên báo giá
              qua Mainflow2.
            </p>
            <Link
              to="/custom-order"
              className="inline-block mt-3 text-xs font-semibold text-emerald-300 hover:text-emerald-200 no-underline"
            >
              Bắt đầu đặt in →
            </Link>
          </div>
      ...
      <section className="rounded-xl bg-gradient-to-r from-slate-800 to-slate-900 text-white p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold m-0">
            Có file riêng hoặc cần AI tạo mô hình?
          </h3>
          <p className="text-sm text-slate-300 mt-2 m-0 max-w-lg">
            Upload STL/GLB, hoặc gửi ảnh để AI tạo GLB — kỹ thuật viên báo giá,
            chat và duyệt giá trực tuyến.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          >
            <div
              className={`h-10 w-10 flex-shrink-0 flex items-center justify-center rounded-full ${item.color} text-white`}
            >
              {item.icon}
            </div>
            <div>
